...
async function callBackend(message: string, userId: string): Promise<ChatResponse> {
    const endpoint = process.env.BACKEND_URL || "http://127.0.0.1:8000/api/report_v1.0";
...
async function main() {
    ...
    console.log(`Using backend: ${process.env.BACKEND_URL || "http://127.0.0.1:8000/api/report_v1.0"}`);
